...
// --- Heartbeat humanizado con jitter aleatorio ---
let heartbeatTimeout = null;
const HEARTBEAT_LOGS = [
  "[Bot] Revisando conexión...",
  "[Bot] Keep-alive...",
  "[Bot] Verificando estado...",
  "[Bot] Ping...",
  "[Bot] Comprobando sesión...",
];
...
async function heartbeatCycle(whatsappClient) {
  if (config.nodeEnv === "test") return;

  try {
    const log =
      HEARTBEAT_LOGS[Math.floor(Math.random() * HEARTBEAT_LOGS.length)];
    console.log(log);

    const roll = Math.random();
    if (roll < 0.15) {
      console.log("[Bot] Sin cambio de presencia...");
    } else if (roll < 0.35) {
      // solo verificar, sin tocar presencia
    } else {
    ...
    if (state !== "CONNECTED") {
      console.warn(`[Bot] Estado anormal: ${state}. Reiniciando...`);
      telegram.sendTelegramAlert(
        `⚠️ *ALERTA DE CONEXIÓN*\n\nEstado anormal (\`${state}\`). Reiniciando...`,
      );
      stopHeartbeat();
      process.exit(1);
    }
  } catch (err) {
    console.error("[Bot] Falló heartbeat:", err.message);
    telegram.sendTelegramAlert(
      `⚠️ *ALERTA DE CONEXIÓN*\n\nError: \`${err.message}\`.\n\nReiniciando...`,
    );
    stopHeartbeat();
...
function scheduleNextHeartbeat(whatsappClient) {
  const delay = randomRange(8 * 60 * 1000, 15 * 60 * 1000);
  heartbeatTimeout = setTimeout(async () => {
    await heartbeatCycle(whatsappClient);
    scheduleNextHeartbeat(whatsappClient);
  }, delay);
}
...
function stopHeartbeat() {
  if (heartbeatTimeout) {
    ...
    console.log("[Bot] Heartbeat detenido.");
  }
}
...
if (config.telegram.pairingNumber) {
  ...
  console.log(
    `[Bot] Configurado para vinculación por código usando el número: ${clientOptions.pairWithPhoneNumber.phoneNumber}`,
  );
}
...
client.on("code", (code) => {
  ...
  const pairingMsg =
    `🔑 *VINCULACIÓN POR CÓDIGO REQUERIDA*\n\n` +
...
client.on("ready", async () => {
  ...
  try {
    ...
    console.error(
      "[Bot] Error al cargar mensajes pendientes locales:",
      err.message,
    );
  }
  ...
  if (hasGoogle) {
    ...
    try {
      const syncData = await google.syncCalendarMessages(
        client,
      ...
      console.error(
        "[Bot] Error en la sincronización inicial del calendario:",
        err.message,
      );
    }
      ...
      setInterval(
        () => {
          google
            ...
            .catch((err) => {
              console.error(
                "[Bot] Error en la sincronización periódica del calendario:",
                err.message,
              );
            });
        },
  ...
  if (startupReport.sent.length > 0 || startupReport.expired.length > 0) {
    ...
    if (startupReport.sent.length > 0) {
      ...
      for (const msg of startupReport.sent) {
        ...
        const timeStr = new Date(msg.scheduledAt).toLocaleString("es-AR", {
...
async function handleMessage(msg) {
  console.log(
    "[Bot] Message event triggered. Body:",
    msg.body,
    "From:",
    msg.from,
    "To:",
...
client.initialize().catch(async (err) => {
  ...
  try {
    ...
    console.error(
      "[Bot] Failed to destroy client on startup failure:",
      destroyErr,
    );
  }
  process.exit(1);
