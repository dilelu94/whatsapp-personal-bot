...
async function sendScheduledMessage(messageObj, dbPath) {
  ...
  try {
    console.log(
      `[Scheduler] Enviando mensaje ${messageObj.id} a ${messageObj.to}...`,
    );

    // Simular comportamiento humano antes de enviar
    const typingTime = randomRange(1500, 5000);
    ...
    if (messageObj.mediaUrl || messageObj.mediaPath) {
      ...
      if (messageObj.mediaUrl) {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to download media from URL: ${messageObj.mediaUrl} (HTTP status ${response.status})`,
          );
        }
        const contentType =
          response.headers.get("content-type") ||
          messageObj.mediaMimeType ||
        ...
        const resolvedPath = path.resolve(
          __dirname,
          "media",
          messageObj.mediaPath,
        );
        if (!fs.existsSync(resolvedPath)) {
          throw new Error(
            `Local media file not found: ${messageObj.mediaPath}`,
          );
        }
        media = MessageMedia.fromFilePath(resolvedPath);
      }
      await whatsappClient.sendMessage(messageObj.to, media, {
    ...
    console.error(
      `[Scheduler] Error sending message ${messageObj.id}:`,
      error.message,
    );
    if (messageObj.status !== "recurring") {
...
function scheduleMessage(messageObj, dbPath) {
  ...
  if (messageObj.status === "recurring" && messageObj.recurrence) {
    ...
    if (messageObj.recurrence.type === "daily") {
      ...
      scheduleRule = {
        dayOfWeek: messageObj.recurrence.dayOfWeek,
        hour: hours,
        minute: minutes,
      };
    }
  ...
  const job = schedule.scheduleJob(scheduleRule, async () => {
    // Jitter humano: 0-30s aleatorios antes de enviar
    const jitter = randomRange(0, 30000);
    await sleep(jitter);
    try {
    ...
    if (msg.status === "pending") {
      ...
      if (scheduledTime <= now) {
        if (scheduledTime >= oneDayAgo) {
          console.log(
            `[Scheduler] Retroactively sending message ${msg.id} (scheduled at ${msg.scheduledAt})...`,
          );
          try {
            ...
            console.error(
              `[Scheduler] Failed to retroactively send message ${msg.id}:`,
              err.message,
            );
          }
        } else {
          db.updateMessageStatus(
            msg.id,
            "failed",
            "Missed schedule (bot was offline > 24h)",
            dbPath,
          );
          report.expired.push({
            to: msg.to,
